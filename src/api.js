...
export async function getTracks(playlistId, progressCallback) {
  return paginatedRequest(
    `users/${state.user.id}/playlists/${playlistId}/tracks`,
    progressCallback
  );
}

export async function getLibraryTracks(progressCallback) {
  return paginatedRequest('me/tracks', progressCallback);
}

function paginatedRequest(url, progressCallback) {
  async function getPage(allItems, url) {
    const { next, limit, items, offset, total } = await request(url);

    if (next) {
      progressCallback(offset + limit, total);
      return getPage([...allItems, ...items], next);
    }
